...
function ConfidenceBar({ value, label }: { value: number; label: string }) {
  return (
    <div className="flex items-center gap-2">
      <span className="text-[11px] md:text-[10px] text-iron-text/45 w-16 shrink-0">{label}</span>
      <div className="flex-1 h-1.5 md:h-1 bg-white/[0.06] rounded-full overflow-hidden">
        <div
          className={`h-full rounded-full transition-all duration-500 ${confidenceColor(value)}`}
          style={{ width: `${value * 100}%` }}
        />
      </div>
      <span className={`text-[11px] md:text-[10px] w-8 text-right font-mono ${confidenceTextColor(value)}`}>
        {(value * 100).toFixed(0)}%
      </span>
    </div>
  );
    ...
    <div className="space-y-4">
      ...
      <div>
        <div className="flex items-center gap-2 mb-1.5">
          <span
            className={`inline-flex items-center gap-1.5 px-2 py-1.5 md:py-1 rounded-md text-[11px] md:text-[10px] font-bold uppercase tracking-wider ${
        ...
        <div className="text-[12px] md:text-[11px] text-iron-text/35 font-mono">
      ...
      <div>
        ...
        <div className="space-y-0.5 max-h-40 md:max-h-32 overflow-y-auto iron-scrollbar no-bounce">
          {analysis.engagement.alerts.map((a, i) => (
            <div key={i} className="text-[12px] md:text-[11px] text-iron-text/50 flex items-center gap-1.5 py-1 md:py-0.5">
...
function SectionHeader({ label }: { label: string }) {
  return (
    <h4 className="text-[11px] md:text-[10px] text-iron-text/35 uppercase tracking-widest font-semibold mb-2">
      {label}
    </h4>
  );
}
...
function Stat({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="bg-white/[0.04] rounded-md px-2.5 py-2.5 md:py-2 border border-white/[0.04]">
      <div className="text-[10px] text-iron-text/30 uppercase tracking-wider">{label}</div>
  ...
  return (
    <>
      <div className="px-3 py-2.5 border-b border-white/[0.06] flex items-center justify-between">
        ...
        <span className="text-[11px] text-iron-text/30 font-mono">
          {analyses.length} eng.
        </span>
      </div>

      {/* Layer toggles */}
      <div className="px-3 py-2 border-b border-white/[0.04] flex gap-1.5 flex-wrap">
      ...
      <div className="flex-1 overflow-y-auto iron-scrollbar no-bounce p-3">
        {selectedAnalysis ? (
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="text-[12px] md:text-[11px] text-iron-text/35 mt-1 font-mono">
          ...
          <div className="space-y-2">
            <p className="text-[12px] md:text-[11px] text-iron-text/30 mb-3">
          ...
          <div className="flex flex-col items-center justify-center h-full gap-3">
            ...
            <div className="text-center">
              ...
              <p className="text-[11px] text-iron-text/15 mt-0.5">Waiting for alert clusters...</p>
            </div>
          </div>
        )}
      </div>
...
function LayerToggle({
  ...
  return (
    <button
      onClick={onClick}
      className={`text-[11px] md:text-[10px] px-3 md:px-2 py-1.5 md:py-1 rounded-md font-medium transition-all duration-200 border min-h-[36px] md:min-h-0 ${
        active
...
function EngagementListItem({ analysis }: { analysis: EngagementAnalysis }) {
  ...
  return (
    <button
      ...
      className={`w-full text-left px-2.5 py-2.5 md:py-2 rounded-md border transition-all duration-150 min-h-[48px] md:min-h-0 ${
        isBallistic
    ...
    >
      <div className="flex items-center justify-between">
        <span
          className={`text-[11px] md:text-[10px] font-bold uppercase tracking-wider ${
            isBallistic ? "text-iron-ballistic" : "text-iron-text/40"
          }`}
        >
          {analysis.classification.replace(/_/g, " ")}
        </span>
        <span className={`text-[11px] md:text-[10px] font-mono font-bold ${confidenceTextColor(analysis.confidence.total)}`}>
          {(analysis.confidence.total * 100).toFixed(0)}%
        </span>
      </div>
      <div className="text-[12px] md:text-[11px] text-iron-text/40 mt-0.5 font-mono">
